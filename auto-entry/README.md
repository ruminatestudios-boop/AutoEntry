...
### Distribution: "Provides mandatory compliance webhooks" / "Verifies webhooks with HMAC" failing

The app already implements mandatory compliance webhooks (`customers/data_request`, `customers/redact`, `shop/redact`) in `shopify.app.toml` and verifies all webhooks with HMAC via `authenticate.webhook(request)`. If Partners automated checks still fail:

1. **Push app config to Partners** so webhook subscriptions are registered:
   ```shell
   npm run deploy
   ```
